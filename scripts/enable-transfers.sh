...
ROUTER_IMPL_ADDRESS=$(jq -r '.transactions | .[] | select(.contractName=="Router") | .contractAddress' $SCRIPT_DIR/../onlyswaps-solidity/broadcast/DeployAllContracts.s.sol/$1/run-latest.json)
ROUTER_ADDRESS=$(jq -r ".transactions | .[] | select((.contractName==\"UUPSProxy\") and (.arguments[0] | test(\"^${ROUTER_IMPL_ADDRESS}$\"; \"i\")?)) | .contractAddress" $SCRIPT_DIR/../onlyswaps-solidity/broadcast/DeployAllContracts.s.sol/$1/run-latest.json)
